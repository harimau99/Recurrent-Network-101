...
def upload_file():
    if request.method == 'POST':
        try:
            ...
            try:
                X_log_temp = np.log1p(X.astype('float32'))
            except:
            ...
            try:
                ...
                for i in range(epoch):
                    ...
                    for n in range(0, (X_log_train.shape[0] // timestamp) * timestamp, timestamp):
                        batch_x = np.expand_dims(X_log_train.iloc[n: n + timestamp, :], axis = 0)
                        batch_y = X_log_train.iloc[n + 1: n + timestamp + 1, :]
                    ...
                    total_loss /= (X_log_train.shape[0] // timestamp)
                    LOSS.append(total_loss)
                    for n in range(0, (X_log_test.shape[0] // timestamp) * timestamp, timestamp):
                        batch_x = np.expand_dims(X_log_test.iloc[n: n + timestamp, :], axis = 0)
                        batch_y = X_log_test.iloc[n + 1: n + timestamp + 1, :]
                        last_state, loss = sess.run([modelnet.last_state, 
                                                        modelnet.cost], feed_dict = {modelnet.X: batch_x, 
                                                                                     modelnet.Y: batch_y, modelnet.hidden_layer: init_value})
                        valid_loss += loss
                        init_value = last_state
                    valid_loss /= (X_log_test.shape[0] // timestamp)
                    ...
                    for n in range(future - 1):
                        ...
                        X_log.loc[X_log.shape[0]] = out_logits[-1, :]
                    X_out = np.expm1(output_predict).T.tolist()
                    ...
                    for n in range(len(columntoselect)):
                        ...
                        quan = quan[0]
                        quan_arange = np.arange(quan.shape[0])
                        regr.fit(quan_arange.reshape([-1, 1]), quan.reshape([-1, 1]))
                        pred = regr.predict(quan_arange.reshape([-1, 1]))
                        quantile_real.append(quan.tolist())
                        arange_real = quan_arange.tolist()
                        ...
                        regr.fit(quan_arange.reshape([-1, 1]), quan.reshape([-1, 1]))
                        pred = regr.predict(quan_arange.reshape([-1, 1]))
                        quantile_predict.append(quan.tolist())
                        arange_predict = quan_arange.tolist()
                    ...
                    del X_T_arr
                    del X_out_arr
                    net_namespace.emit('senddata', json.dumps({'id': idsocket, 'data': {'color': color_array, 'ori': X_T, 'ori-range': x_range_original, 'predict-range': x_range_future, 
                                                                                        'epoch': EPOCH, 'loss': LOSS, 'valid': VALID_LOSS, 'columns': columntoselect, 'predict': X_out, 'status': status,
                                                                                       'quantile-real': quantile_real, 'arange_real': arange_real, 'linear_real': linear_real,
                                                                                       'quantile_predict': quantile_predict, 'arange_predict': arange_predict, 'linear_predict': linear_predict,
