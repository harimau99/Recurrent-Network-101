...
function setgraph(data){
  ...
  for(var i = 0; i < data['columns'].length; i++){
    ...
    if(i == 0){
      var scatter_real =  {
        x: data['arange_real'],
        y: data['quantile-real'][i],
        mode: 'markers',
        name: 'QQ ' + data['columns'][i]
      };
      var scatter_predict =  {
        x: data['arange_predict'],
        y: data['quantile_predict'][i],
        mode: 'markers',
        name: 'QQ ' + data['columns'][i]
      };
      var line_real =  {
  ...
  for(var i = 0; i < data['columns'].length; i++){
    if(i == 0){
      layout_qq_real['xaxis'] = {domain: [0, ((i + 1) * split) - boundary]}
      layout_qq_predict['xaxis'] = {domain: [0, ((i + 1) * split) - boundary]}
      layout_bar_real['xaxis'] = {domain: [0, ((i + 1) * split) - boundary]}
      layout_bar_predict['xaxis'] = {domain: [0, ((i + 1) * split) - boundary]}
    }
    else{
      layout_qq_real['yaxis' + (i + 1)] = {anchor: 'x' + (i + 1)}
      layout_qq_predict['yaxis' + (i + 1)] = {anchor: 'x' + (i + 1)}
      layout_qq_real['xaxis' + (i + 1)] = {domain: [(i * split) + boundary, (i + 1) * split]}
      layout_qq_predict['xaxis' + (i + 1)] = {domain: [(i * split) + boundary, (i + 1) * split]}
      layout_bar_real['yaxis' + (i + 1)] = {anchor: 'x' + (i + 1)}
      layout_bar_predict['yaxis' + (i + 1)] = {anchor: 'x' + (i + 1)}
      layout_bar_real['xaxis' + (i + 1)] = {domain: [(i * split) + boundary, (i + 1) * split]}
      layout_bar_predict['xaxis' + (i + 1)] = {domain: [(i * split) + boundary, (i + 1) * split]}
    }
  }
  var trace_loss = {
    x: data['epoch'],
